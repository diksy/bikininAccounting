...
<head>
    ...
    <title><?= htmlspecialchars($title ?? 'Create Journal Entry') ?></title>
    ...
    <main class="mx-auto max-w-4xl px-6 py-16">
        <div class="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
            <div class="mb-6">
                ...
                <h1 class="mt-2 text-3xl font-bold text-slate-900">Create journal entry</h1>
            </div>

            <form method="post" action="/accounting/journal-entries/create" class="space-y-6">
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        <label for="entry_no" class="mb-2 block text-sm font-medium text-slate-700">Entry No</label>
                        <input id="entry_no" name="entry_no" type="text" value="<?= htmlspecialchars($generatedEntryNo) ?>" readonly class="w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2.5 text-slate-700 focus:border-slate-500 focus:outline-none" />
                    </div>
                    <div>
                        <label for="date" class="mb-2 block text-sm font-medium text-slate-700">Date</label>
                        <input id="date" name="date" type="date" required class="w-full rounded-lg border border-slate-300 px-3 py-2.5 focus:border-slate-500 focus:outline-none" />
                    </div>
                </div>

                <div>
                    <label for="description" class="mb-2 block text-sm font-medium text-slate-700">Description</label>
                    <input id="description" name="description" type="text" required class="w-full rounded-lg border border-slate-300 px-3 py-2.5 focus:border-slate-500 focus:outline-none" />
                </div>
                ...
                <div class="space-y-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <div class="grid gap-4 md:grid-cols-5">
                        <div class="md:col-span-2">
                            <label class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500">Account</label>
                        </div>
                        <div>
                            <label class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500">Note</label>
                        </div>
                        <div>
                            <label class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500">Debit</label>
                        </div>
                        <div>
                            <label class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500">Credit</label>
                        </div>
                    </div>
                        ...
                        <div class="journal-line grid gap-4 md:grid-cols-5">
                            <select name="line_account_id[]" class="md:col-span-2 rounded-lg border border-slate-300 px-3 py-2.5 focus:border-slate-500 focus:outline-none">
                                ...
                                <?php foreach ($accounts as $account): ?>
                                    <option value="<?= htmlspecialchars($account->id) ?>"><?= htmlspecialchars($account->id) ?> - <?= htmlspecialchars($account->name) ?></option>
                                <?php endforeach; ?>
                            </select>
                            <input type="text" name="line_note[]" placeholder="Description" class="rounded-lg border border-slate-300 px-3 py-2.5 focus:border-slate-500 focus:outline-none" />
                            <input type="number" min="0" name="line_debit[]" value="0" class="rounded-lg border border-slate-300 px-3 py-2.5 focus:border-slate-500 focus:outline-none" />
                            <input type="number" min="0" name="line_credit[]" value="0" class="rounded-lg border border-slate-300 px-3 py-2.5 focus:border-slate-500 focus:outline-none" />
                        </div>
                    </div>
                ...
                <div class="flex gap-3">
                    <button type="submit" class="flex-1 rounded-lg bg-slate-900 px-4 py-3 text-sm font-semibold text-white hover:bg-slate-700">Save entry</button>
